package com.keepgulp.taobaoschedulelearn.cronexpression;
...
public class CronExpression implements Serializable, Cloneable {
    ...
    protected String getExpressionSetSummary(java.util.Set set) {
        ...
        if (!itr.hasNext() && lastdayOfMonth) {
            buf.append("you set last day of the month, the day will be got until a defined time we get");
        }
        if (!itr.hasNext() && monLdaySet != null && monLdaySet.isLast) {
            buf.append("'L' is in your month setting area, the day will be got until a defined time we get");
        }
        boolean first = true;
        while (itr.hasNext()) {
        ...
        while (!gotOne) {
            ...
            if (dayOfMSpec && !dayOfWSpec) { // get day by day of month rule
                ...
                if (monLdaySet != null && monLdaySet.isLast && !endDaysFlag) {
                    int end = getLastDayOfMonth(mon, cl.get(Calendar.YEAR)) - monLdaySet.ex + 1;
                    if (monLdaySet.value == 0) {
                        endDays = end;
                    } else {
                        try {
                            if (monLdaySet.value > end) {
                                throw new IllegalArgumentException("Start day must be less than end day");
                            }
